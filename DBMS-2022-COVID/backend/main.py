...
class Hospitaldata(db.Model):
    ...
    covaxin = db.Column(db.Integer)
    covishield = db.Column(db.Integer)
    sputnik = db.Column(db.Integer)
...
class Bookingpatient(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    srfid = db.Column(db.String(20), unique=True)
    vactype = db.Column(db.String(100))
    hcode = db.Column(db.String(20))
...
def hospitalUser():
    ...
    if('user' in session and session['user'] == params['user']):
        ...
        if request.method == "POST":
          ...
          #  mail.send_message('COVID VACCINATION SLOT BOOKING', sender=params['gmail-user'], recipients=[
...
def logoutadmin():
    session.pop('user')
    flash("Logout from admin ", "primary")
...
def addhospitalinfo():
    ...
    if request.method == "POST":
        ...
        covaxin = request.form.get('covaxin')
        covishield = request.form.get('covishield')
        sputnik = request.form.get('sputnik')
        hcode = hcode.upper()
        ...
        if huser:
            db.engine.execute(
                f"INSERT INTO `hospitaldata` (`hcode`,`hname`,`covaxin`,`covishield`,`sputnik`) VALUES ('{hcode}','{hname}','{covaxin}','{covishield}','{sputnik}')")
...
def hedit(id):
    ...
    if request.method == "POST":
        ...
        hcode = hcode.upper()
        db.engine.execute(
            f"UPDATE `hospitaldata` SET `hcode`='{hcode}',`hname`='{hname}',`covaxin`='{covaxin}',`covishield`='{covishield}',`sputnik`='{sputnik}' WHERE `hospitaldata`.`id`='{id}'")
        flash(" Slot updated", "sucess")
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        vactype = request.form.get('vactype')
        hcode = request.form.get('hcode')
        ...
        elif vactype == "covishield":
            for d in dbb:
                seat = d.covishield
                print(seat)
                ar = Hospitaldata.query.filter_by(hcode=code).first()
                ar.covishield = seat-1
                db.session.commit()

        elif vactype == "sputnik":
            for d in dbb:
                seat = d.sputnik
                print(seat)
                ar = Hospitaldata.query.filter_by(hcode=code).first()
        ...
        if(seat > 0 and check):
            res = Bookingpatient(srfid=srfid, vactype=vactype, hcode=hcode,
                                 dose=dose, pname=pname, pphone=pphone, paddress=paddress)
            db.session.add(res)
            db.session.commit()
